...
<key>\n\tA string used to encrypt or decrypt the message.\n")
    ...
    textin = ""
    while(textin != "QUIT"):
        if (not sys.stdin.isatty()): # Redirect input if stdin is present
            textin = sys.stdin.read()
            redirected = True 
        else: # If not, regular input
            textin = input("")

        if (textin == "QUIT"): # Inorder to have a gracefull shutdown
           sys.exit("")
        ...
        # if file was redirected in, then exit program to avoid infinite loop
        if redirected:
            sys.exit("")
